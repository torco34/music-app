<template>
  <q-layout view="hHr LpR lfr" class="bg-green-radial">
    <q-header class="flex bg-header q-mt-md q-mr-xl items-center ">
      <div class="menu-contariner q-mr-lg" :class="{ 'menu-open': !$q.screen.lt.md && leftDrawerOpen }">
        <q-btn color="primary" dense flat round icon="menu" size="16px" @click="leftDrawerOpen = !leftDrawerOpen"
          class="btn-layout" />
      </div>
      <div class="blur-green-bg col flex text-white items-center q-pb-md">
        <q-input borderless v-model="search" input-class="" class="q-mx-lg col text-white "
          placeholder="Buscar tu musica">
          <template v-slot:prepend>
            <q-icon v-if="search === ''" name="search" />
            <q-icon v-else name="clear" class="cursor-pointer  text-white" @click="search = ''" />
          </template>
        </q-input>
        ...
        <div class="flex items-center">
          ...
          <q-btn round dense flat class="q-mx-xl icons-style" icon="notifications">
    ...
    <q-drawer show-if-above v-model="leftDrawerOpen" side="left" class="bg-secondary" :width="247">
      <div class="custom-left-menu q-pb-md text-center">
        ...
        <q-list>
          ...
          <MenuItemLink v-for="link in musicMenuLinks" :key="link.title" elevated v-bind="link" />
    ...
    <q-drawer show-if-above v-model="rightDrawerOpen" side="right" :width="168" class="bg-secondary">
      texto blanco
    </q-drawer>
